...
add_action('admin_menu', function () {
    add_menu_page('Telegram Upload', 'TE File Upload', 'manage_options', 'telegram-file-upload', function () {
        ...
        echo '<pre class="bg-gray-50 p-3 rounded text-xs overflow-x-auto">curl -X POST https://' . $_SERVER['HTTP_HOST'] . '/wp-json/telegram-upload/v1/upload \\
  -H "X-API-Key: ' . esc_attr($api_key) . '" \\
  -F "file=@document.pdf"</pre>';
        echo '<div class="mt-2">';
        echo '<a href="' . plugins_url('docs/API_QUICK_START.md', dirname(__FILE__)) . '" target="_blank" class="text-blue-600 hover:underline">📖 View Full API Documentation</a>';
        echo '</div>';
        echo '</div>';
        echo '</div>';
