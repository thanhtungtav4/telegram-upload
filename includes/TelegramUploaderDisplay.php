    ...
    public static function render_uploaded_files($wpdb, $table, $api_base, $token, $is_frontend = false) {
        ...
        foreach ($rows as $entry) {
            ...
            if (!empty($entry->max_downloads)) {
                ...
                echo '<span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ' . $badge_color . '" title="Downloads: ' . ($entry->access_count ?? 0) . '/' . $entry->max_downloads . '">';
                echo $limit_reached ? '🚫 Limit' : '📊 ' . $remaining . '/' . $entry->max_downloads;
                echo '</span>';
            }
