...
class TelegramAsyncUploader {
    ...
    public static function handle_async_upload() {
        ...
        // Store metadata
        $metadata = [
            'category' => isset($_POST['telegram_category']) ? sanitize_text_field(wp_unslash($_POST['telegram_category'])) : '',
            'tags' => isset($_POST['telegram_tags']) ? sanitize_text_field(wp_unslash($_POST['telegram_tags'])) : '',
            'description' => isset($_POST['telegram_description']) ? sanitize_textarea_field(wp_unslash($_POST['telegram_description'])) : '',
            'expiration_date' => isset($_POST['telegram_expiration']) ? sanitize_text_field(wp_unslash($_POST['telegram_expiration'])) : '',
            'password' => isset($_POST['telegram_password']) ? sanitize_text_field(wp_unslash($_POST['telegram_password'])) : '',
    ...
    private function send_to_telegram_with_progress($path, $filename, $pending_id) {
        ...
        // phpcs:ignore WordPress.WP.AlternativeFunctions.curl_curl_exec -- Required for progress tracking with CURLOPT_PROGRESSFUNCTION
        $result = curl_exec($ch);
