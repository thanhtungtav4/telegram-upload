...
class GeneralSettings {
    ...
    public static function init() {
        ...
        add_action('admin_enqueue_scripts', function($hook) {
            // Check if we're on settings page using $_GET parameter
            $is_settings_page = (
                isset($_GET['page']) && 
                sanitize_text_field(wp_unslash($_GET['page'])) === 'te-file-upload-settings'
            );
            ...
            wp_localize_script('telefiup-admin-settings', 'telefiupSettings', array(
                'ajaxurl' => admin_url('admin-ajax.php'),
                'nonce' => wp_create_nonce('telefiup_test_telegram')
            ));
        });
