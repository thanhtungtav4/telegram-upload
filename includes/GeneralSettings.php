...
class GeneralSettings {
    ...
    public static function init() {
        ...
        add_action('admin_enqueue_scripts', function($hook) {
            ...
            wp_enqueue_style('tailwindcdn', 'https://cdn.jsdelivr.net/npm/tailwindcss@2.2.19/dist/tailwind.min.css', [], null);
        });
        ...
        <div class="wrap">
            ...
            <form method="post" action="options.php">
                ...
                <table class="form-table">
                    <tr><th>Bot Token</th><td><input type="text" name="<?= self::OPTION_NAME ?>[token]" value="<?= esc_attr($token) ?>" size="60" /></td></tr>
                    <tr><th>Chat ID</th><td><input type="text" name="<?= self::OPTION_NAME ?>[chat_id]" value="<?= esc_attr($chat_id) ?>" size="60" /></td></tr>
                    <tr>
                        <th>API Base URL</th>
                        <td>
                            <input type="text" name="<?= self::OPTION_NAME ?>[api_base]" value="<?= esc_attr($api_base) ?>" size="60" />
                            <br />
                    ...
                    <tr>
                        ...
                        <td>
                            <label>
                                <input type="radio" name="<?= self::OPTION_NAME ?>[upload_method]" value="server-side" <?= checked($upload_method, 'server-side', false) ?> />
                            ...
                            <br /><br />
                            <label>
                                <input type="radio" name="<?= self::OPTION_NAME ?>[upload_method]" value="client-side" <?= checked($upload_method, 'client-side', false) ?> />
            ...
            <script>
            document.getElementById('te-send-test').addEventListener('click', function() {
                const btn = this;
