...
class TelegramClientUpload {
    ...
    async upload(file, metadata = {}) {
        try {
            ...
            if (file.size === 0) {
                throw new Error('File is empty. Telegram does not accept empty files.');
            }
    ...
    async uploadToTelegram(file, tokenData) {
        return new Promise((resolve, reject) => {
            // Check file size
            if (file.size > tokenData.max_file_size) {
                reject(new Error(`File too large. Maximum size is ${tokenData.max_file_size / 1024 / 1024}MB`));
                return;
            }
            ...
            xhr.addEventListener('load', () => {
                if (xhr.status === 200) {
                    ...
                    try {
                        ...
                        reject(new Error(`Telegram API error: ${errorResponse.description || xhr.status}`));
                    } catch (e) {
